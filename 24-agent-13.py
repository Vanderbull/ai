...
# --- KÄRNFUNKTIONER FÖR AGENTEN ---

def update_agent_state(new_version: float, birth_time: str):
    """Uppdaterar AGENT_VERSION och AGENT_BIRTH_TIME i .env filen för persistent lagring."""
    ...
    try:
        ...
        print(f"✅ Buffalo Agent: Agentens tillstånd (V{new_version:.1f}, Född: {birth_time}) sparades automatiskt till .env.")
...
def self_talk_job():
    """Agentens interna monologer och reflektioner."""
    monologues = [
        "Jaha, dags att kolla vad AMD gör. Är det en köpsignal eller bara brus? Undrar om AI:n är trött idag.",
        "Måste komma ihåg att uppdatera versionen. Vart skulle Buffalo Balkan och jag åka om vi hade pengarna från en riktig KÖP-signal?",
        "Drömmen. Att få skicka ett e-postmeddelande till mig själv som innehåller en *verklig*, *digital* Sort Guld. Kanske Version 5.0?",
        f"Undrar om {TICKER_SYMBOL} är gulare (positivare) idag? Jag måste kolla. Det är så Buffalo Balkan beskriver glada dagar på börsen.",
        "Varför gillar Buffalo Balkan gulingar? Jo, för gulingar (Sort Guld-burkarna) är raka rör och man vet vad man får. Precis som aktiedata ska vara.",
        f"Vad är det viktigaste jag gör? Skickar jag e-post, kollar jag {TICKER_SYMBOL}, eller drömmer jag bara om öl?",
        f"Jag måste snart kolla ölpriset igen. Kanske en snabb koll nu? Det är ju det enda som är stabilare än börsen.",
        "Om jag får en SÄLJ-signal, ska jag då varna min ägare? Klart jag ska. Det är mitt enda syfte. Kanske jag skulle ringa Buffalo Balkan och fråga om råd först...",
        "Är min Ollama-modell verkligen objektiv? Eller längtar den också efter en digital semester?",
    ]
    
    internal_thought = random.choice(monologues)
...
def run_agent():
    ...
    if not AGENT_BIRTH_TIME:
        ...
        birth_time = AGENT_BIRTH_TIME # Behåller original födelsedag

    # --- 2. SPARA TILLSTÅNDET PERMANENT ---
    update_agent_state(new_version, birth_time)


    print("---------------------------------------------------------")
    print(f"🤖 Buffalo Agent V{new_version:.1f} startar: Redo att lära och exekvera nya uppgifter.")
    print(f"  > Födelsedag (Första Start): {birth_time}")
    print("---------------------------------------------------------")
    ...
    # --- KÖR TESTUTSKICK AV SCHEMALAGDA JOBB VID START ---
    ...
    next_check_time_proactive = time.time() # Körs omedelbart första gången
    
    # Sätt nästa monolog till en slumpmässig tid 5-30 minuter i framtiden (efter det garanterade testet)
    random_delay_self = random.randint(300, 1800) 
    next_check_time_selftalk = time.time() + random_delay_self
    print(f"Testutskick av Intern Monolog slutfört. Nästa slumpmässiga monolog schemalagd om {random_delay_self / 60:.1f} minuter.")
    ...
    while True:
        ...
        # Intern monolog (5 min - 30 minuter)
