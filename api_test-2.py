...
MAX_RETRIES_UNIQUE_CPU = 50 # Antal försök för att hitta en unik CPU i köpcykeln
...
# --- DATABAS PÅFYLLNING (BULK - NU ITERATIV) ---
...
def populate_database_with_generic_data(db: AgentDB, client: ollama.Client):
    ...
    while True:
        ...
        # Hämta alla kända CPU-namn för exkludering i prompten
        existing_cpus = db.get_all_cpu_names()
        
        # Begränsa listan av exklusioner för att inte överskrida promptstorleken
        if len(existing_cpus) > 50: 
        ...
        # Systemprompt för batchhämtning
        list_prompt_system = (
        ...
        for cpu_name in cpu_list:
            if cpu_name in existing_cpus:
                # CPU finns redan i de kända CPU-namnen, ignorera och gå vidare.
                continue
            ...
            if details:
                try:
                    # Lägg till i databasen och uppdatera räknare
                    ...
                    existing_cpus.add(cpu_name) # Uppdatera kända CPUer direkt
...
# --- KÄRNFUNKTIONER (KÖPCYKEL - Oförändrad från V14) ---

def analyze_and_upgrade_hardware_v14(db: AgentDB):
    """Agentens huvudfunktion: Analysera hårdvara, rekommendera och köp (simulerat), med ökat antal försök."""
    ...
    print("\n--- 🤖 SystemAgent V15: Hårdvaruanalys & Köp (Ollama) Startad ---")
    ...
    try:
        ...
        # --- Steg 2: Iterativ Rekommendation med Dubbelkoll mot DB (50 försök) ---
        print(f"\n--- Steg 2: SystemAgent Ber om Bättre CPU (JSON) & Dubbelkollar DB ({MAX_RETRIES_UNIQUE_CPU} försök)... ---")
        
        MAX_RETRIES = MAX_RETRIES_UNIQUE_CPU
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if attempt == MAX_RETRIES - 1:
                print("❌ Max antal försök nått (50). Alla föreslagna CPUer finns redan i DB. Avbryter köpcykeln.")
                return 
        ...
        print("\n--- Steg 3: Hämta detaljerade specifikationer och pris via LLM/DB... ---")
...
if __name__ == "__main__":
    ...
    try:
        ...
        analyze_and_upgrade_hardware_v14(db)
        
        # 2. Fyll på databasen med generell information (Nu iterativ tills LLM inte kan hitta unika)
