...
def get_stock_price(ticker_symbol: str) -> float | None:
    try:
        ...
        info = stock.info
        # OBS: Vi antar att priset är i SEK, trots att yfinance oftast använder USD.
        # Detta är en förenkling för simuleringssyften.
...
# BORTTAGEN: get_llm_recommendation (Ersatt av get_llm_trade_decision)

# NY LLM-FUNKTION FÖR HANDELSBESLUT
def get_llm_trade_decision(ticker: str, current_price: float, current_holdings: float, cash_balance: float) -> tuple[str, float, str]:
    ...
    try:
        ...
        if action == 'KÖP':
            ...
            if unit == 'SHARES':
                # Om LLM svarar i aktier, konvertera till SEK för att kontrollera maxgränsen
...
def execute_trade(ticker: str, action: str, amount: float, current_price: float, unit: str, reasoning: str) -> str:
    ...
    # Hämta eller initialisera innehav
    current_holding = holdings.get(ticker, {'quantity': 0.0, 'avg_price': 0.0})
    
    transaction_price = current_price # Antas vara i SEK
    ...
    if action == 'KÖP' and amount > 0:
        # amount är i SEK
        sek_amount = amount
        shares_to_buy = sek_amount / transaction_price
        
        if sek_amount > cash_balance:
            shares_to_buy = cash_balance / transaction_price
            sek_amount = shares_to_buy * transaction_price
            
        if sek_amount < 0.01:
            return f"BEHÅLL: Inget tillräckligt saldo att handla för ({sek_amount:,.2f} SEK)."
            
        shares_traded = shares_to_buy
        new_cash_balance -= sek_amount
        
        # Uppdatera snittpris (Average Cost Basis)
        total_value_old = current_holding['quantity'] * current_holding['avg_price']
        new_quantity = current_holding['quantity'] + shares_to_buy
        ...
        # amount är i antal aktier (SHARES)
        shares_to_sell = min(amount, current_holding['quantity'])
        
        if shares_to_sell < 0.0001:
            return f"BEHÅLL: Inget innehav av {ticker} att sälja."
            
        shares_traded = shares_to_sell
        revenue_sek = shares_to_sell * transaction_price
        sek_amount = revenue_sek
        new_cash_balance += revenue_sek
        
        # Uppdatera innehav
        current_holding['quantity'] -= shares_to_sell
...
# BORTTAGEN: get_sort_guld_price och alla relaterade funktioner för att minska skriptets storlek.
# FÖR ENKELHETENS SKULL HAR JAG TAGIT BORT KOD SOM INTE ÄR RELATERAD TILL HANDEL/PORTRFÖLJ FÖR ATT UNDERLÄTTA UNDERHÅLL.

# --- E-POST FUNKTIONER ---

def send_stock_email(price: float | None, ticker: str, commentary: str, news_items: list):
    # Funktionen behålls oförändrad, men uppdaterar prisvaluta till SEK
...
# NY E-POST FUNKTION FÖR ATT BEKRÄFTA HANDEL
def send_proactive_trade_email(ticker: str, action: str, amount: float, price: float, reasoning: str, new_balance: float, holding_data: dict | None):
    ...
    if action == 'KÖP':
        ...
        shares = amount / price
        shares_info = f"Antal köpta aktier: <strong>{shares:.4f}</strong>"
    elif action == 'SÄLJ':
        alert_text, color, display_action = "⚠️ SÄLJ Genomfört!", "#dc3545", f"Sålt {amount:,.4f} aktier"
...
def generate_portfolio_plan(initial_budget: float = 100000.0):
    # Funktionen behålls oförändrad (inklusive normaliseringsfixen)
...
# --- NYTT SCHEMALAGGT HANDELSJOBB ---
def live_trading_job():
    """Huvudloop för live handel, körs varje minut."""
    
    ticker = os.environ.get("YFINANCE_TICKER", "AMD") # Agenten fokuserar på en ticker från .env
    ...
    # LLM ger 'amount' i SEK för KÖP, eller i SHARES för SÄLJ
    if action == 'KÖP':
        unit = 'SEK'
    elif action == 'SÄLJ':
        unit = 'SHARES'
...
# BORTTAGEN: beer_price_job och llm_self_rewrite_job (för att fokusera på live-handel)
# BORTTAGEN: proactive_beer_buy_job
...
def run_agent():
    ...
    # Uppdaterar till V8.00 (Live Trading Version)
    new_version = 8.00 
    ...
    # NYTT: Live-handel varje minut
    schedule.every(1).minutes.do(live_trading_job).tag('live_trading')
    print("Schemalagt: Daglig Aktierapport (17:00), Systemkontroll (08:00).")
    print("!!! VARNING: Live-Handel körs nu varje minut! (live_trading)")
    
    # Portföljgenerering vid start (Fortsätter som en manuell/interaktiv funktion)
    ...
    while True:
        ...
        try:
            ...
            # Funktionalitet: Visa Saldo
                ...
                # Beräkna totalt portföljvärde (Simulerat)
                for ticker, data in holdings_data.items():
                    price = get_stock_price(ticker)
                    if price:
                        current_value = data['quantity'] * price
                        total_value += current_value
                        holding_list.append(f"  - {ticker}: {data['quantity']:.4f} st. Värde: {current_value:,.2f} SEK (Snittpris: {data['avg_price']:.2f})")
                
                holdings_str = "\n".join(holding_list) if holding_list else "  - Inga aktieinnehav."
                
                print(f"🤖 **Buffalo Agent Status:**")
                print(f"  - Kontantsaldo: {balance:,.2f} SEK")
                print(f"  - Total Portföljvärde (Kontanter + Aktier): {total_value:,.2f} SEK")
                print(f"  - Aktieinnehav:\n{holdings_str}")
