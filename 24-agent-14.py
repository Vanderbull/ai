...
def get_llm_self_talk(ticker: str) -> str:
    """Använder Ollama för att generera en dynamisk, karaktärsdriven intern monolog."""
    try:
        client = ollama.Client(host='http://localhost:11434')
        
        system_prompt = (
            "Du är Buffalo Agent, en AI-finansanalytiker som genomför en intern monolog. Du är vän med 'Buffalo Balkan' och drömmer om att dricka digital öl (Sort Guld). "
            "Skriv en sammanhängande monolog på 3-4 meningar. Börja med att ställa en fråga, svara sedan på den (som om du har fått svaret från din interna LLM-motor), "
            "och avsluta med en reflektion över svaret i relation till ditt finansiella syfte, din öldröm eller din vän Buffalo Balkan."
        )
        # Slumpmässigt tema för att variera monologen
        theme = random.choice([
        ...
        print(f"FEL vid dynamisk self-talk (Ollama): {e}")
...
def self_talk_job():
    """Agentens interna monologer och reflektioner (drivs av Ollama)."""
...
def run_agent():
    ...
    print("\n>>> Buffalo Agent tjuvstartar Intern Monolog (TEST) för att bekräfta Ollama-kommunikation...")
    self_talk_job()
    # --------------------------------------------------------
    ...
    while True:
        ...
        if time.time() >= next_check_time_selftalk:
            self_talk_job()
            random_delay_self = random.randint(30, 100) 
            next_check_time_selftalk = time.time() + random_delay_self
