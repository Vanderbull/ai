...
def get_current_hardware_info() -> dict:
    ...
    simulated_processor = "Intel Core i5-7200U (Initial/Simulated)" # Ändrat till en typisk mobil CPU
...
def fetch_cpu_details_from_rapidapi(component_name: str) -> dict | None:
    ...
    # ... (resten av RapidAPI-logiken är oförändrad) ...
...
def fetch_initial_laptop_model(client: ollama.Client, hardware_info: dict) -> str: # NY FUNKTION (V29)
    """Använder LLM för att bestämma det exakta modellnamnet på den bärbara datorn."""
...
def populate_database_with_generic_data(db: AgentDB, client: ollama.Client):
    ...
    for component_type in DESKTOP_COMPONENT_TYPES + LAPTOP_COMPONENT_TYPES: 
        ...
        while True:
            ...
            # ... (resten av bulk-logiken är oförändrad) ...
...
# --- KÄRNFUNKTIONER (KÖPCYKEL - UPPDATERAD V29) ---

def analyze_and_upgrade_hardware_v29(db: AgentDB, client: ollama.Client, system_type: str, max_budget: float) -> bool:
    """
    ...
    # OBS: Initialiseringen (att sätta current_CPU/Laptop) görs nu i __main__
    ...
    # Logik för att hämta detaljer/pris för den föreslagna komponenten
    # ... (oförändrad från V28) ...
    
    # ... (resten av köplogiken är oförändrad) ...

    # --- Köplogik (Hämtar pris och hanterar transaktion) ---
...
def run_upgrade_cycle(db: AgentDB, client: ollama.Client, system_type: str, max_budget: float):
    """Kör den kontinuerliga uppgraderingscykeln (oförändrad)."""
    ...
    while True:
        ...
        # Använd den uppdaterade analysfunktionen
        purchase_successful = analyze_and_upgrade_hardware_v29(db, client, system_type, max_budget)
...
def generate_summary(db: AgentDB, initial_budget: float, final_budget: float):
    ...
    print("\n\n=======================================================")
    print("🚀 SLUTLIG SYSTEMSAMMANSTÄLLNING OCH EKONOMI (V29)")
    print("=======================================================")
    ...
    system_type_flag = db.get_current_component_name("Laptop")
    system_type = "Laptop" if system_type_flag else "Desktop"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if system_type == "Laptop":
            ...
            # **NY LOGIK** Få det exakta Laptop-modellnamnet
