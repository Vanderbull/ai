...
def get_stock_price(ticker_symbol: str, target_time: datetime.datetime | None = None) -> float | None:
    ...
    if target_time is None:
        ...
        try:
            ...
            # Försöker med smalare till bredare intervall för bästa precision
            intervals = ['1m', '1h', '1d']
            lookbacks = [datetime.timedelta(hours=2), datetime.timedelta(days=1), datetime.timedelta(days=7)] # 7 days för att täcka helger
            
            for interval, lookback in zip(intervals, lookbacks):
                start_time = target_time - lookback
                
                # yfinance API tar inte emot datetime-objekt direkt. Vi formaterar.
                start_str = start_time.strftime('%Y-%m-%d %H:%M:%S')
                end_str = target_time.strftime('%Y-%m-%d %H:%M:%S')
                
                # Dagligt intervall får inte ha tid specificerad i end-parametern
                if interval == '1d':
                    end_str = target_time.strftime('%Y-%m-%d')
            ...
            # --- AGGRESSIV GARANTERAD FALLBACK (30 dagar) ---
            # Om 1m, 1h och 7 dagars historik misslyckas, hämta en månads daglig data
            # för att garantera att vi får det senast tillgängliga priset.
            start_time_final = target_time - datetime.timedelta(days=30)
            
            df_final = yf.download(
                ticker_symbol, 
                start=start_time_final.strftime('%Y-%m-%d'), 
                end=target_time.strftime('%Y-%m-%d'),
                interval='1d', 
                progress=False 
            )
            
            if not df_final.empty:
                return df_final['Close'].iloc[-1] # Senaste priset i 30-dagarsfönstret
            
        except Exception as e:
            # Allvarligt fel i yfinance-anropet
            print(f"DEBUG: Allvarligt yfinance-fel vid hämtning av historik: {e}")
            pass # Låt funktionen returnera None nedan
...
def get_price_history(ticker_symbol: str, agent_time: datetime.datetime, lookback_days: int = 2) -> pd.DataFrame:
    ...
    try:
        ...
        history = stock.history(
            ...
            end=agent_time.strftime('%Y-%m-%d %H:%M:%S') # End date is agent_time
        )
        return history
    except Exception as e:
        return pd.DataFrame() 
...
def execute_trade(ticker: str, action: str, amount: float, current_price: float, unit: str, reasoning: str) -> str:
    ...
    *** Viktigt: Kontantsaldot dras/läggs till i new_cash_balance och sparas till .env via update_agent_state(). ***
    ...
    current_version = float(os.environ.get("AGENT_VERSION", "8.85"))
