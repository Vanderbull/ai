...
# --- TIDHANTERING (AGENTENS INTERNA KLOCKA) ---
def get_agent_time() -> datetime.datetime:
    """Returnerar agentens interna tid, vilket är exakt 24 timmar bakåt i tiden."""
...
def get_stock_price(ticker_symbol: str, target_time: datetime.datetime | None = None) -> float | None:
    ...
    if target_time is None:
        ...
        try:
            stock = yf.Ticker(ticker_symbol)
            # Fetch 1-minute data covering a short period up to the target time
            start_time = target_time - datetime.timedelta(hours=2) # Fetch enough data for a few hours
            ...
            if not history.empty:
                # Get the last available close price (closest to target_time)
...
def execute_trade(ticker: str, action: str, amount: float, current_price: float, unit: str, reasoning: str) -> str:
    ...
    *** Viktigt: Kontantsaldot dras/läggs till i new_cash_balance och sparas till .env via update_agent_state(). ***
    ...
    current_version = float(os.environ.get("AGENT_VERSION", "8.81"))
...
def print_portfolio_status():
    ...
    agent_time = get_agent_time() # Agentens tid
...
def live_trading_job():
    """Huvudloop för live handel, körs var 30:e sekund baserat på agentens tid."""
    
    ticker = os.environ.get("YFINANCE_TICKER", "AMD")
    agent_time = get_agent_time() # Agentens interna tid

    print(f"\n--- Buffalo Agent: Utför LIVE HANDEL ({ticker}) vid (Agentstid: {agent_time.strftime('%Y-%m-%d %H:%M:%S')}) ---")
    
    # Hämta priset vid agentens tidpunkt
...
def run_agent():
    ...
    agent_time = get_agent_time() # Hämta agentens simulerade tid
    ...
    # Uppdaterar till V8.81 (Bekräftelse av uppdatering av kassa vid handel)
    new_version = 8.81 
    ...
    print(f"🕒 Agentens interna tid är {agent_time.strftime('%Y-%m-%d %H:%M:%S')}")
