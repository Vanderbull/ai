...
import datetime 
import re 
import pandas as pd # NY IMPORT: Krävs för tidsseriedata
...
def get_sentiment_score(title: str) -> float:
    ...
    try:
        ...
        # Försök konvertera till float
        score = float(score_str)
        if -1.0 <= score <= 1.0:
            return score
        return 0.0 # Fallback om LLM ger ett ogiltigt nummer
...
def get_price_history(ticker_symbol: str, lookback_days: int = 2) -> pd.DataFrame:
    ...
    try:
        stock = yf.Ticker(ticker_symbol)
        # Hämta 2 dagars 1-timmarsdata för att täcka de senaste 24h händelserna
...
def get_recent_news(ticker_symbol: str) -> list:
    ...
    try:
        ...
        # Hämta prisdata för analys
        price_history = get_price_history(ticker_symbol)
        if price_history.empty:
            print("Varning: Kunde inte hämta prisdata för prispåverkan.")

        for item in news_list:
            publish_timestamp = None
            
            # --- 1. Hämta Tidsstämpel (Robust hantering av båda formaten) ---
            ...
            try:
                ...
                if publish_time > cutoff_time:
                    ...
                    # --- 2. Prispåverkansanalys ---
                    price_change_percent = None
                    if not price_history.empty:
                        
                        release_dt = pd.to_datetime(publish_time, utc=True) # Konvertera till Pandas datetime, antag UTC
                        
                        # Pris vid release (P_release)
                        P_release = price_history.asof(release_dt)['Close']
                        
                        # Pris en timme senare (P_hour_later)
        ...
        # --- 4. Sortera: Från mest positivt (högst poäng) till mest negativt ---
...
def send_stock_email(price: float | None, ticker: str, commentary: str, news_items: list):
    ...
    if news_items:
        ...
        for item in news_items:
            # Enklare version av nyhetsvisning för daglig rapport
...
def send_proactive_email(price: float | None, ticker: str, action: str, reasoning: str, check_type: str, news_items: list):
    ...
    # --- NYHETSRANKNING OCH PRISDISPLAY FÖR NOTISER ---
    news_html = ""
    if action == 'NOTIS' and news_items:
        ...
        for i, item in enumerate(news_items):
            ...
            if item['price_change_percent'] is not None:
                ...
                price_info = f'<span style="color: {color}; font-weight: bold;">{sign}{change:.2f}%</span> (1h efter release)'
...
def daily_reporting_job():
    ...
    # Nyheterna hämtas här, men den utökade analysen används inte för den dagliga rapporten.
...
def pro_active_check_job():
    ...
    price = get_stock_price(TICKER_SYMBOL)
    # Observera: get_recent_news inkluderar nu sentiment- och prisanalys.
