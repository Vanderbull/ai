...
# --- KÄRNFUNKTIONER FÖR AKTIER (OFÖRÄNDRADE) ---
...
def get_sort_guld_price() -> tuple[float | None, str]:
    ...
    try:
        print(f"Buffalo Agent hämtar data från: {URL}")
...
def send_stock_email(price: float | None, ticker: str, commentary: str, news_items: list):
    ...
    try:
        ...
        print(f"✅ Buffalo Agent har skickat Aktierapporten! Done!")
    except Exception as e:
...
def send_proactive_email(price: float | None, ticker: str, action: str, reasoning: str, check_type: str, news_items: list):
    ...
    try:
        ...
        server.login(SMTP_USER, SMTP_PASS)
        server.sendmail(SMTP_USER, MAIL_TO, msg.as_string())
        print(f"✅ PROAKTIV VARNING SKICKAD AV BUFFALO AGENT!")
    except Exception as e:
...
def send_beer_price_email(price: float | None, search_snippet: str):
    ...
    if price is not None:
        ...
        status_text = f"Det aktuella priset för Sort Guld är: <strong>{price_str}</strong>. Billigt? Dyrt? Vem vet!"
    else:
        price_str = "Ej tillgängligt"
        subject = "❓ BUFFALO AGENT: Kunde INTE få tag på ölpriset idag!"
        status_text = "Jäkla webbplats. Kunde inte fastställa det aktuella priset för Sort Guld."
    ...
    <html>
      <body>
        ...
        <p><small>Buffalo Agent skickar denna rapport dagligen kl 10:00. Deal with it.</small></p>
      </body>
    </html>
    """
    msg.attach(MIMEText(html_body, 'html'))
    ...
    try:
        ...
        server.login(SMTP_USER, SMTP_PASS)
        server.sendmail(SMTP_USER, MAIL_TO, msg.as_string())
        print(f"✅ Buffalo Agent har skickat ölprisrapporten! Dagens mission slutförd!")
    except Exception as e:
...
def daily_reporting_job():
    ...
    print(f"\n--- BUFFALO AGENT: Jag MÅSTE köra DAGLIG RAPPORT NU! ({time.strftime('%Y-%m-%d %H:%M:%S')}) ---")
...
def beer_price_job():
    ...
    print(f"\n--- BUFFALO AGENT: Dags att kolla ölpriset! NU KÖR VI! ({time.strftime('%Y-%m-%d %H:%M:%S')}) ---")
...
def pro_active_check_job():
    ...
    print(f"\n--- BUFFALO AGENT: Proaktiv Kontroll startar (Fokus: {check_type})! Rör på er! ---")
    ...
    if check_type == 'PRICE':
        if price is not None:
            ...
            if recommendation in ['KÖP', 'SÄLJ']:
                print(f"** BUFFALO AGENT: SIGNAL UPPFÅNGAD! {recommendation} (Pris) **")
                send_proactive_email(price, TICKER_SYMBOL, recommendation, reasoning, 'PRIS', [])
            else:
                print(f"Buffalo Agent avstår. Beslut: {recommendation}. Motivering: {reasoning}")
        else:
            print("Kontrollen hoppades över: Prisdata ej tillgänglig. Tråkgt!")
        ...
        if news_items:
            ...
            print(f"** BUFFALO AGENT: HÄR KOMMER NYHETERNA! (Direkt Notis) **")
            send_proactive_email(price, TICKER_SYMBOL, action, reasoning, 'NYHETER', news_items)
        else:
            print("Kontrollen hoppades över: Inga nya nyheter hittades. Pfft.")


def run_agent():
    """Huvudloopen som kör agenten kontinuerligt."""
    print("---------------------------------------------------------")
    print("🤖 BUFFALO AGENT startar! Jag är på jakt efter data. Se upp för plötsliga utbrott...")
    print("---------------------------------------------------------")

    # Schemaläggning
    schedule.every().day.at("17:00").do(daily_reporting_job).tag('daily_stock')
    print("Schemalagt: Daglig Aktierapport körs kl 17:00 CET. (Tråkigt men nödvändigt).")
    
    schedule.every().day.at("10:00").do(beer_price_job).tag('daily_beer')
    print("Schemalagt: Daglig Ölpriskontroll (Sort Guld) körs kl 10:00 CET. (Det viktiga jobbet).")
    
    # --- KÖR TESTUTSKICK AV ÖLKOLLEN VID START ---
    print("\n>>> Buffalo Agent tjuvstartar Ölpriskontroll (TEST)...")
    beer_price_job()
    print("Testutskick av Ölpriskontroll slutfört. (Japp, det funkar).")
    # ----------------------------------------------
    
    next_check_time = time.time() 
    
    print("Buffalo Agent går in i STANDBY. Jag VET att du inte gör något viktigt nu ändå.")

    while True:
        schedule.run_pending()
        
        # 10% chans att skrika ut "FÅGEL!!!"
        ...
        if time.time() >= next_check_time:
            ...
            print(f"Buffalo Agent tar en paus. Nästa proaktiva aktiekontroll schemalagd om {delay_minutes:.1f} minuter.")

        time.sleep(1)

if __name__ == "__main__":
    if not all([SMTP_HOST, SMTP_USER, SMTP_PASS, MAIL_TO, TICKER_SYMBOL]):
        print("❌ FEL: Nödvändiga miljövariabler saknas. Kontrollera .env-filen. DU GÖR ETT DÅLIGT JOBB!")
    else:
        run_agent()
