...
def get_llm_trade_decision(ticker: str, current_price: float, current_holdings: float, cash_balance: float) -> tuple[str, float, str]:
    ...
    try:
        ...
        system_prompt = (
            ...
            "Du kan använda hela ditt kontantsaldo (100%) för ett köp om du anser det nödvändigt, men för att uppnå ditt 1% dagliga vinstmål bör du fokusera på mindre, snabba affärer. Du säljer max 50% av ditt innehav per transaktion." # ÄNDRAT: Tog bort 5%-gränsen för köp
        )
        user_prompt = (
        ...
        if action == 'KÖP':
            ...
            if unit == 'SHARES':
                ...
                if sek_cost > max_buy_sek:
                    amount = max_buy_sek
                    unit = 'SEK'
                    reasoning += " (Justering: Begränsad köp till max kontantsaldo)." # ÄNDRAT motivering
...
def execute_trade(ticker: str, action: str, amount: float, current_price: float, unit: str, reasoning: str) -> str:
    ...
    current_version = float(os.environ.get("AGENT_VERSION", "8.40"))
...
def run_agent():
    ...
    # Uppdaterar till V8.40 (Ta bort 5% köpgräns)
    new_version = 8.40 
    ...
    if True: # Återställ alltid till 100k och noll innehav vid start av V8.40
