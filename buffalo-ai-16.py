...
def get_stock_price(ticker_symbol: str, target_time: datetime.datetime | None = None) -> float | None:
    ...
    Inkluderar fallback-mekanism (1m -> 1h -> 1d) för att hantera gles historisk data.
    """
    if target_time is None:
        ...
        try:
            ...
            # 1. Försök 1-minutsintervall (bäst precision)
            start_time_1m = target_time - datetime.timedelta(hours=2) 
            history_1m = stock.history(
                interval='1m', 
                start=start_time_1m.strftime('%Y-%m-%d %H:%M:%S'), 
                end=target_time.strftime('%Y-%m-%d %H:%M:%S')
            )
            
            if not history_1m.empty:
                return history_1m['Close'].iloc[-1]
            
            # 2. Fallback: Försök 1-timmarsintervall 
            start_time_1h = target_time - datetime.timedelta(days=1)
            history_1h = stock.history(
                interval='1h', 
                start=start_time_1h.strftime('%Y-%m-%d %H:%M:%S'), 
                end=target_time.strftime('%Y-%m-%d %H:%M:%S')
            )
            
            if not history_1h.empty:
                 # Ta det senaste tillgängliga priset
                return history_1h['Close'].iloc[-1] 
                
            # 3. Fallback: Försök Dagligt intervall 
            start_time_1d = target_time - datetime.timedelta(days=3)
            history_1d = stock.history(
                interval='1d', 
                start=start_time_1d.strftime('%Y-%m-%d'), 
                end=target_time.strftime('%Y-%m-%d') # End date is the day of agent_time
            )
            
            if not history_1d.empty:
                # Ta stängningspriset från den dagen
                return history_1d['Close'].iloc[-1] 
...
def execute_trade(ticker: str, action: str, amount: float, current_price: float, unit: str, reasoning: str) -> str:
    ...
    *** Viktigt: Kontantsaldot dras/läggs till i new_cash_balance och sparas till .env via update_agent_state(). ***
    ...
    current_version = float(os.environ.get("AGENT_VERSION", "8.84"))
