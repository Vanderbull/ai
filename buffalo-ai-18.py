...
def get_stock_price(ticker_symbol: str, target_time: datetime.datetime | None = None) -> float | None:
    ...
    if target_time is None:
        ...
        try:
            ...
            # --- 1. FÖRSÖK 1M/1H (Hög precision) ---
            intervals = ['1m', '1h'] 
            # Vi tittar 2 timmar och 1 dag bakåt för att hitta en candle nära target_time
            lookbacks = [datetime.timedelta(hours=2), datetime.timedelta(days=1)] 
            
            for interval, lookback in zip(intervals, lookbacks):
                start_time = target_time - lookback
                
                # Använd YYYY-MM-DD HH:MM för att undvika problem med sekunder (som orsakade felet)
                start_str = start_time.strftime('%Y-%m-%d %H:%M')
                end_str = target_time.strftime('%Y-%m-%d %H:%M')
                
                # Försök 1: yf.Ticker.history
                ...
                if not history.empty:
                    # Viktigt: Hämta det sista tillgängliga stängningspriset i intervallet (närmast target_time).
                    return history['Close'].iloc[-1]
                    
            # --- 2. AGGRESSIV GARANTERAD FALLBACK (30 dagar, 1d interval) ---
            # Detta säkerställer att vi alltid får ett pris om 1m/1h misslyckas (t.ex. under helger).
            ...
            if not df_final.empty:
                # Välj det senaste priset i 30-dagarsfönstret
...
def get_price_history(ticker_symbol: str, agent_time: datetime.datetime, lookback_days: int = 2) -> pd.DataFrame:
    ...
    try:
        ...
        history = stock.history(
            ...
            end=agent_time.strftime('%Y-%m-%d %H:%M') # FIX: Bort med sekunder även här
        )
        return history
    except Exception as e:
        return pd.DataFrame() 
...
def execute_trade(ticker: str, action: str, amount: float, current_price: float, unit: str, reasoning: str) -> str:
    ...
    *** Viktigt: Kontantsaldot dras/läggs till i new_cash_balance och sparas till .env via update_agent_state(). ***
    ...
    current_version = float(os.environ.get("AGENT_VERSION", "8.86"))
