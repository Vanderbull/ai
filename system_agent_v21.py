...
# --- HJÄLPFUNKTIONER (UPPDATERAD V20 LOGIK) ---
...
# --- KÄRNFUNKTIONER (KÖPCYKEL - UPPDATERAD V21) ---

def analyze_and_upgrade_hardware_v21(db: AgentDB):
    """Agentens huvudfunktion: Analysera hårdvara, rekommendera och köp (simulerat), inklusive detaljerad systeminformation."""
    ...
    # Använd den mest detaljerade processornamnet som finns
    current_processor = full_hardware_info.get('Processor', 'Unknown CPU')
    current_balance = db.get_balance()
    
    print("\n--- 🤖 SystemAgent V21: Hårdvaruanalys & Köp (Detaljerad systeminfo) Startad ---")
    ...
    try:
        ...
        # Samma meddelande, men nu kan agenten och användaren se att utvärderingen baseras på detaljer
        print("\n--- Steg 1: LLM Utvärderar Hårdvaran... ---")
        print(f"  > Baserat på detaljer hämtade via {full_hardware_info.get('Hardware_Info_Source', 'standardmetoder')}, är {current_processor} utvärderad som...")
        print(f"  > (Utvärdering: Bra, men för svag för krävande AI-arbetslaster.)") # LLM:en skulle normalt utföra denna bedömning här
        ...
        # UPPDATERAD PROMPT: Inkludera all systeminformation till LLM:en
        user_prompt_2 = (
            f"Föreslå en uppgradering till min nuvarande processor utifrån följande specifikationer:\n\n"
            f"Systemspecifikationer:\n{spec_list}\n\n"
            f"Min nuvarande processor är: {current_processor}"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        analyze_and_upgrade_hardware_v21(db)
