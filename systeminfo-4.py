...
import requests 
import subprocess # NY IMPORT FÖR SYSTEMKOMMANDON
...
# --- HJÄLPFUNKTIONER (UPPDATERAD V20) ---
...
def get_current_hardware_info() -> dict:
    ...
    if info["OS"] == "Linux":
        try:
            ...
            info["Processor"] = info.get("CPU_Model_name", info.get("CPU_Architecture", "Unknown Linux Processor"))
            info["CPU_Cores"] = info.get("CPU_CPU(s)", "N/A")
            info["Hardware_Info_Source"] = "Linux (lscpu)"

        except FileNotFoundError:
            info["Processor"] = platform.processor() or "AMD Ryzen 5 3600 (Simulated - lscpu not found)"
            info["CPU_Cores"] = os.cpu_count() or "N/A"
            info["Hardware_Info_Source"] = "Fallback (lscpu not installed)"
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as e:
            info["Processor"] = platform.processor() or "AMD Ryzen 5 3600 (Simulated - lscpu failed)"
            info["CPU_Cores"] = os.cpu_count() or "N/A"
            info["Hardware_Info_Source"] = f"Fallback (lscpu error: {type(e).__name__})"
        
    else: # Fallback för macOS, Windows eller andra OS
...
# --- KÄRNFUNKTIONER (KÖPCYKEL - UPPDATERAD V20) ---

def analyze_and_upgrade_hardware_v20(db: AgentDB):
    ...
    print("\n--- 🤖 SystemAgent V20: Hårdvaruanalys & Köp (Detaljerad systeminfo) Startad ---")
    ...
    # NY UTSKRIFT: Visa detaljerad systeminformationen
...
if __name__ == "__main__":
    ...
    try:
        ...
        analyze_and_upgrade_hardware_v20(db)
