...
class AgentDB:
    ...
    def log_purchase(self, item_name: str, item_type: str, cost: float):
        # Uppdaterad för att logga component_type istället för hårdkodad 'CPU'
...
def fetch_cpu_details_from_rapidapi(component_name: str) -> dict | None:
    ...
    # Förenklad version för denna simulerade agent.
    # I en riktig agent skulle denna del kräva anpassning baserat på API-slutpunkt och parsning.
    
    if not RAPIDAPI_HOST or not RAPIDAPI_KEY:
        return None
        
    # Notera: Denna URL är simulerad
    ...
    try:
        ...
        # Simulerad parsning av API-svar (Antar att API:et returnerar 'price_sek')
...
def fetch_component_specs_from_llm(client: ollama.Client, component_name: str, component_type: str) -> dict | None:
    ...
    # Försök att ge LLM:en vägledning om vilka specifikationer den ska inkludera
...
def populate_database_with_generic_data(db: AgentDB, client: ollama.Client):
    ...
    for component_type in COMPONENT_TYPES:
        ...
        while True:
            ...
            # Skapa exkluderingslista för prompten
            ...
            list_prompt_system = (
                ...
                f"Fokusera på nya och olika modeller. Svara ENDAST med ett JSON array av strängar: [\"Modell Namn 1\", \"Modell Namn 2\", ...]. "
            ...
            for component_name in component_list:
                ...
                # Använder den generaliserade hämtningsfunktionen
...
# --- KÄRNFUNKTIONER (KÖPCYKEL - FOKUS PÅ CPU BEHÅLLS FÖR NU) ---

def analyze_and_upgrade_hardware_v18(db: AgentDB):
    """Agentens huvudfunktion: Analysera hårdvara, rekommendera och köp (simulerat)."""
    
    hardware_info = platform.processor() or "AMD Ryzen 5 3600"
    current_processor = hardware_info
    current_balance = db.get_balance()
    
    print("\n--- 🤖 SystemAgent V18: Hårdvaruanalys & Köp (Fokus CPU) Startad ---")
    ...
    try:
        ...
        recommended_type = suggestion_data.get('component_type') # Ska vara 'CPU'
        ...
        if is_new_component:
            # Komponent är ny, hämta detaljerna från LLM och RapidAPI och logga.
            ...
            if detailed_data:
             ...
             # Komponent finns, hämta detaljerna från DB för snabbhet/konsistens.
             ...
             # Konvertera tuple till dict för enkel hantering
             detailed_data = {
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 1. Kör den vanliga köp/analyscykeln (fortfarande fokuserad på CPU)
        analyze_and_upgrade_hardware_v18(db)
        
        # 2. Fyll på databasen med generell information (Nu iterativt över alla komponenttyper)
        ...
        if all_details:
            ...
            for row in all_details:
                # Försök att parsa JSON och presentera de första två specifikationerna
