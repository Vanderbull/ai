...
# Används för initial återställning i AgentDB.__init__
...
class AgentDB:
    ...
    # NY FUNKTION: För att uppdatera budget efter systemtyp-detektering
...
def get_current_hardware_info() -> dict:
    ...
    simulated_processor = "AMD Ryzen 5 3600 (Initial/Simulated)"
...
def detect_system_type(client: ollama.Client, hardware_info: dict) -> str: # NY FUNKTION
    """Använder LLM för att avgöra om det är Desktop eller Laptop."""
    ...
    system_prompt = (
        ...
        "Basera din slutsats på namn på CPU, Model name (om tillgängligt), och andra systemdetaljer som kan tyda på mobilitet (t.ex. 'Battery' i detaljer). "
...
def populate_database_with_generic_data(db: AgentDB, client: ollama.Client):
    ...
    # Kör både för Desktop-komponenter och Laptop-system
        ...
        while True:
            ...
            try:
                ...
                if not isinstance(component_list, list) or not component_list:
                    # Stoppa loopen om LLM inte hittar mer av denna typ
...
# --- KÄRNFUNKTIONER (KÖPCYKEL - UPPDATERAD V28) ---

def analyze_and_upgrade_hardware_v28(db: AgentDB, client: ollama.Client, system_type: str, max_budget: float) -> bool:
    """
    ...
    # Bestäm vilka komponenttyper som ska spåras (Laptop vs Desktop)
    if system_type == "Laptop":
        tracked_types = LAPTOP_COMPONENT_TYPES
        # Initial setup: Sätt den simulerade processorn som den "nuvarande laptopen"
        initial_component_name = full_hardware_info.get('Processor', 'Unknown Laptop')
        if not db.get_current_component_name("Laptop"):
             db.set_current_component_name("Laptop", initial_component_name)
             # Logga initial info för att möjliggöra försäljning/detaljanalys
             if not db.check_if_component_exists(initial_component_name):
                 initial_details = fetch_component_specs_from_llm(client, initial_component_name, "Laptop")
                 if initial_details:
                     db.log_hardware_details(initial_details)
    else:
        tracked_types = DESKTOP_COMPONENT_TYPES
        # Initial setup för Desktop
        initial_component_name = full_hardware_info.get('Processor', 'Unknown CPU')
        if not db.get_current_component_name("CPU"):
             db.set_current_component_name("CPU", initial_component_name)
             if not db.check_if_component_exists(initial_component_name):
                 initial_details = fetch_component_specs_from_llm(client, initial_component_name, "CPU")
                 if initial_details:
                     db.log_hardware_details(initial_details)
            ...
            if details:
                ...
                for key, value in details.items():
                    if key not in ['component_name', 'component_type', 'price_sek', 'details_json']:
                        # För Laptop kan det bli många fält, lägg till dem direkt
                        if comp_type == "Laptop":
...
def run_upgrade_cycle(db: AgentDB, client: ollama.Client, system_type: str, max_budget: float):
    ...
    while True:
        ...
        purchase_successful = analyze_and_upgrade_hardware_v28(db, client, system_type, max_budget)
    ...
    print("\n--- SystemAgent V28 Avslutar ---")

def generate_summary(db: AgentDB, initial_budget: float, final_budget: float):
    """Genererar en sammanställning av alla transaktioner och systemets slutliga tillstånd."""
    
    print("\n\n=======================================================")
    print("🚀 SLUTLIG SYSTEMSAMMANSTÄLLNING OCH EKONOMI (V28)")
    print("=======================================================")
    ...
    system_type = "Laptop" if final_budget == LAPTOP_BUDGET else "Desktop"
    ...
    if system_type == "Laptop":
        ...
        if current_name:
            ...
            if details:
                ...
                # Visa några nyckel-specs för laptopen
    ...
    all_transactions = sorted(purchases + sales, key=lambda x: x[3]) # Sortera efter datum

    for item, item_type, amount, date in all_transactions:
        if item_type in DESKTOP_COMPONENT_TYPES + LAPTOP_COMPONENT_TYPES: # Purchase
            print(f"  [KÖP] -{amount:,.2f} kr: {item} ({item_type}) @ {date}")
        else: # Sale
             # Återanvänd logiken, men måste veta om det är ett köp eller en försäljning.
             # Vi kan använda att försäljningar sparas i sales-tabellen.
             # Eftersom jag slog ihop listorna, måste jag särskilja dem.
             # Bättre att iterera över purchases och sedan sales.
             pass # Skip combined loop
             
    # Rätt logik för utskrift
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 2. Sätt rätt budget och uppdatera DB
        if system_type == "Laptop":
            max_budget = LAPTOP_BUDGET
            db.set_balance(max_budget)
        else:
            max_budget = DESKTOP_BUDGET
            db.set_balance(max_budget) # Återställer till DESKTOP_BUDGET (om DB:n nollades)
